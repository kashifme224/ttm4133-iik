...
def get_tput(exa):
    ...
    t1 = pd.DataFrame({'time': time, 'tput': tput1, 'uid': 'u11'}, columns=['time', 'tput', 'uid'])
    t2 = pd.DataFrame({'time': time, 'tput': tput2, 'uid': 'u12'}, columns=['time', 'tput', 'uid'])
    t3 = pd.DataFrame({'time': time, 'tput': tput3, 'uid': 'u13'}, columns=['time', 'tput', 'uid'])
    
    result = pd.concat([t1, t2, t3], keys=["u11", "u12", "u13"])
...
if(display):
    ...
    print('####################################################')
    preprocess_file(ran_logs)
    print('\n')
    print('####################################################')
    print('EPC Connection Setup Logs [SGW, PGW, MME]')
    print('####################################################')
    preprocess_file(epc_logs)
